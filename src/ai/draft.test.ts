...
const baseInput = (overrides: Partial<DraftInput> = {}): DraftInput => ({
  ...
  beats: [
    { title: '안산역 도착', line: '김올가가 안산역에 내린다.', sources: ['인물 — 김올가'], path: '비트/비트 — 안산역 도착.md' },
    { title: '강제이주 회상', line: '화물열차를 회상한다.', sources: ['인터뷰 — 김올가 20260601'], path: '비트/비트 — 강제이주 회상.md' }
  ],
  cards: [
    { title: '인물 — 김올가', path: '카드/인물 — 김올가.md', summary: '1938년생.', evidence: '1938년 연해주 출생. 안산 정착.' },
    { title: '인터뷰 — 김올가 20260601', path: '카드/인터뷰 — 김올가 20260601.md', summary: '회고.', evidence: '1937년 강제이주를 증언.' }
  ],
  ...overrides
});
...
describe('normalizeDraftSections', () => {
  it('limits a section to the sources of its own beat (whitelist within beat)', () => {
    const sections = normalizeDraftSections(baseInput(), {
      sections: [{ beatTitle: '안산역 도착', prose: '김올가가 안산역 계단을 내려온다.', sources: ['인물 — 김올가', '인터뷰 — 김올가 20260601'] }]
    });
    expect(sections).toHaveLength(1);
    expect(sections[0].sources).toEqual(['인물 — 김올가']);
  });

  it('drops a section whose beatTitle does not match an approved beat', () => {
    const sections = normalizeDraftSections(baseInput(), {
      sections: [{ beatTitle: '없는 비트', prose: '아무 말.', sources: ['인물 — 김올가'] }]
    });
  ...
  it('drops a section with empty prose', () => {
    const sections = normalizeDraftSections(baseInput(), {
      sections: [{ beatTitle: '안산역 도착', prose: '   ', sources: ['인물 — 김올가'] }]
    });
    expect(sections).toHaveLength(0);
  });
  ...
  it('blocks a nonfiction section with no valid sources', () => {
    const sections = normalizeDraftSections(baseInput(), {
      sections: [{ beatTitle: '안산역 도착', prose: '김올가가 내린다.', sources: [] }]
    });
  ...
  it('flags a year in the prose that is not in the beat or cited card', () => {
    const sections = normalizeDraftSections(baseInput(), {
      sections: [{ beatTitle: '안산역 도착', prose: '1945년 광복 직후 안산역에 내렸다.', sources: ['인물 — 김올가'] }]
    });
  ...
  it('does not flag a year present in the cited evidence', () => {
    const sections = normalizeDraftSections(baseInput(), {
      sections: [{ beatTitle: '강제이주 회상', prose: '1937년의 강제이주를 떠올린다.', sources: ['인터뷰 — 김올가 20260601'] }]
    });
...
describe('selectSavableDraftSections (export boundary)', () => {
  it('excludes a section bound to a non-existent beat', () => {
    const sections = [{ beatTitle: '없는 비트', prose: '글.', sources: ['인물 — 김올가'], flags: [], blocking: false }];
  ...
  it('excludes a card that exists globally but is not in the section beat sources', () => {
    const crossBeat = [
      { beatTitle: '안산역 도착', prose: '글.', sources: ['인터뷰 — 김올가 20260601'], flags: [], blocking: false }
    ];
  ...
  it('excludes a nonfiction section with no real source cards and returns sourceCards otherwise', () => {
    ...
    const ok = [{ beatTitle: '안산역 도착', prose: '글.', sources: ['인물 — 김올가'], flags: [], blocking: false }];
...
describe('revalidateDraftSections', () => {
  it('recomputes flags when an edit adds a new ungrounded year', () => {
    const edited = [{ beatTitle: '안산역 도착', prose: '2002년 월드컵 무렵 안산역에 내렸다.', sources: ['인물 — 김올가'], flags: [], blocking: false }];
  ...
  it('drops a section edited to blank prose', () => {
    const edited = [{ beatTitle: '안산역 도착', prose: '   ', sources: ['인물 — 김올가'], flags: [], blocking: false }];
